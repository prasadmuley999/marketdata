...
EXCEL_FILE_BASE = "NSE_Merged_Reports"

def get_dated_excel_filename():
    return f"{EXCEL_FILE_BASE}_{datetime.now().strftime('%d-%b-%Y')}.xlsx"
...
def generate_html_content(df, dates, chronological_indices, gainers, losers):
    ...
    header_row_1 += '  <th rowspan="2" onclick="sortTable(0)" class="px-4 py-3 text-left cursor-pointer hover:bg-slate-700 min-w-[140px] vertical-align-middle sticky left-0 z-20 bg-slate-800">Symbol</th>\n'
    ...
    for _, row in df.iterrows():
        sym = row['SYMBOL']
        rows_html += f'<tr class="group border-b border-slate-700 hover:bg-slate-800 transition-colors">\n'
        rows_html += f'  <td class="px-4 py-3 text-sm font-bold text-slate-100 sticky left-0 z-10 bg-slate-900 group-hover:bg-slate-800">{sym}</td>\n'    
...
def write_to_excel_workbook(master_df, valid_filenames, dates, chronological_indices, excel_file):
...
def send_email_dashboard(recipient, html_content, email_body_html, excel_file):
...
def main():
    ...
    excel_file = get_dated_excel_filename()
    write_to_excel_workbook(master_df, valid_filenames, dates, chronological_indices, excel_file)
    ...
    if recipient:
        ...
        send_email_dashboard(recipient, html_content, email_body_html, excel_file)
    else:
